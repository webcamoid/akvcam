...
bool akvcam_buffers_mapped(akvcam_buffers_t self);
int akvcam_buffers_write_frame_to_buffers(akvcam_buffers_t self,
                                          akvcam_frame_t frame);
int akvcam_buffers_write_frame_to_rw_buffers(akvcam_buffers_t self,
                                             akvcam_frame_t frame);
                            ...
                            struct v4l2_requestbuffers *params)
{
    ...
    if (params->count < 1) {
        ...
        for (i = 0; i < params->count; i++) {
            ...
            result = akvcam_buffer_read(buffer, &v4l2_buff);
            ...
            result = akvcam_buffer_write(buffer, &v4l2_buff);
...
void akvcam_buffers_deallocate(akvcam_buffers_t self)
{
    ...
    result = akvcam_buffers_resize_rw(self, self->rw_buffer_size);
                          ...
                          akvcam_format_ct format)
{
    ...
    if (buffers->count > 0) {
        ...
        if (last_buffer && akvcam_buffer_read(last_buffer, &v4l2_buff) == 0)
                         ...
                         struct v4l2_buffer *buffer)
{
    ...
    if (akbuffer
        && akvcam_buffer_read(akbuffer, &v4l2_buff) == 0
        ...
        if (buffer->type == V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE
            ...
            result = akvcam_buffer_write(akbuffer, &v4l2_buff);
        }
...
int akvcam_buffers_queue(akvcam_buffers_t self, struct v4l2_buffer *buffer)
{
    ...
    if (buffer->memory == V4L2_MEMORY_USERPTR && !self->multiplanar) {
        if (!buffer->m.userptr) {
            akpr_err("The pointer is NULL.\n");

            return -EINVAL;
        }

        if (!akvcam_userptr_is_valid((char __user *) buffer->m.userptr,
                                     buffer->length)) {
            akpr_err("Invalid memory address.\n");
    ...
    if (akbuffer) {
        result = akvcam_buffer_read(akbuffer, &v4l2_buff);

        if (result == 0) {
            if (v4l2_buff.flags & V4L2_BUF_FLAG_QUEUED) {
                mutex_unlock(&self->buffers_mutex);
                akpr_err("Buffer is already queued.\n");
            ...
            if (v4l2_buff.type == buffer->type) {
                ...
                case V4L2_MEMORY_USERPTR:
                        ...
                        v4l2_buff.flags &= (__u32) ~(V4L2_BUF_FLAG_MAPPED
                                                     | V4L2_BUF_FLAG_DONE);
                        result = akvcam_buffer_write_userptr(akbuffer, &v4l2_buff);

                        if (result != 0)
                            akpr_err("Can't write buffer data.\n");
...
static akvcam_buffer_t akvcam_buffers_next_buffer(akvcam_buffers_ct self)
{
    ...
    for (;;) {
        ...
        if (akvcam_buffer_read(buffer, &v4l2_buff) == 0
...
int akvcam_buffers_dequeue(akvcam_buffers_t self, struct v4l2_buffer *buffer)
{
    ...
    if (result) {
        akpr_err("Failed to lock buffers.\n");
    ...
    if (akbuffer) {
        if (akvcam_buffer_read(akbuffer, &v4l2_buff) == 0) {
            if (v4l2_buff.type == buffer->type) {
                if (buffer->type == V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE
                    || buffer->type == V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE) {
                    v4l2_buff.length = buffer->length;
                ...
                case V4L2_MEMORY_USERPTR:
                    result = akvcam_buffer_read_userptr(akbuffer, &v4l2_buff);
...
akvcam_frame_t akvcam_buffers_read_frame(akvcam_buffers_t self)
{
    ...
    if (mutex_lock_interruptible(&self->buffers_mutex) == 0) {
        ...
        if (!akvcam_list_empty(self->buffers)) {
            ...
            if (buffer && akvcam_buffer_read(buffer, &v4l2_buff) == 0) {
...
{
    int result = 0;
    bool buffers_empty = akvcam_list_empty(self->buffers);

    akpr_function();

    if (self->rw_mode & (AKVCAM_RW_MODE_MMAP | AKVCAM_RW_MODE_USERPTR)
        && !buffers_empty) {
        result = akvcam_buffers_write_frame_to_buffers(self, frame);
    } else if (frame
               && self->rw_mode & AKVCAM_RW_MODE_READWRITE
               && buffers_empty) {
        result = akvcam_buffers_write_frame_to_rw_buffers(self, frame);
    } else {
...
void akvcam_buffers_reset_flags(akvcam_buffers_t self)
{
    ...
    for (;;) {
        ...
        if (akvcam_buffer_read(buffer, &v4l2_buff) == 0) {
            ...
            if (akvcam_buffer_write(buffer, &v4l2_buff) != 0)
                akpr_err("Failed writing buffer.\n");
        } else {
            akpr_err("Can't read buffer.\n");
        }
...
bool akvcam_buffers_mapped(akvcam_buffers_t self)
{
    ...
    for (;;) {
        ...
        if (akvcam_buffer_mapped(buffer))
            return true;
    }
                                          ...
                                          akvcam_frame_t frame)
{
    ...
    akpr_function();
    result = mutex_trylock(&self->buffers_mutex);
